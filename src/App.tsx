...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFB] text-[#1A1C19] font-sans selection:bg-emerald-100 selection:text-emerald-900 pb-36 overflow-x-hidden">
      {/* Refined Header */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#FDFDFB]/80 backdrop-blur-2xl border-b border-emerald-950/5 px-6">
        <div className="max-w-xl mx-auto h-24 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <motion.div 
              whileHover={{ rotate: 90 }}
              className="w-10 h-10 rounded-full bg-emerald-950 flex items-center justify-center shadow-xl shadow-emerald-950/20"
            ...
            >
              <h1 className="text-xl font-bold tracking-tight text-emerald-950 font-sans group-hover:text-emerald-700 transition-colors">VibeCycle</h1>
              <span className="text-[10px] uppercase tracking-[0.2em] font-black text-emerald-600/60">Transformasi Limbah Menjadi Karya</span>
            </div>
          </div>
          
          <motion.div 
            whileHover={{ y: -2 }}
            className="flex items-center gap-3 sm:gap-4 pl-3 sm:pl-4 border-l border-emerald-950/5"
          >
            <div className="text-right">
              <p className="text-[9px] font-black text-emerald-700 uppercase tracking-[0.2em]">{currentBadge.name}</p>
              <p className="text-xs sm:text-sm font-sans font-medium text-slate-500">{points} Eco-Pts</p>
            </div>
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white border border-emerald-950/5 shadow-sm flex items-center justify-center text-emerald-700">
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'scan' ? (
              ...
              {!image ? (
                <div className="space-y-12">
                  <div className="space-y-6">
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="inline-block px-4 py-1.5 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-black uppercase tracking-[0.3em]"
                    >
                    ...
                    <motion.h2 
                      ...
                      className="text-5xl sm:text-7xl font-bold font-serif leading-[0.9] text-emerald-950"
                    >
                      Beri Peluang Baru<br />
                      <span className="italic font-light text-emerald-600/80">Untuk Semesta.</span>
                    </motion.h2>
                    <p className="text-slate-500 text-lg sm:text-xl font-medium leading-relaxed max-w-sm">
                      Identifikasi sampah & temukan ide kreatif <span className="text-emerald-950">Upcycle</span> yang ditenagai AI.
                    </p>
                  </div>
                  ...
                  <motion.div 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="aspect-[5/4] w-full rounded-[3.5rem] bg-white border border-emerald-950/5 shadow-2xl shadow-emerald-900/5 flex flex-col items-center justify-center gap-8 cursor-pointer group relative overflow-hidden transition-all duration-700"
                  ...
                  >
                    ...
                    <div className="relative z-10 text-center space-y-2">
                      <p className="text-2xl font-bold font-serif text-emerald-950 group-hover:tracking-tight transition-all">Scan Objek Sampah</p>
                ...
                <div className="space-y-12">
                  <div className="space-y-4">
                    <div className="relative aspect-[4/3] w-full rounded-[3rem] overflow-hidden shadow-2xl bg-[#1A1C19] border border-emerald-950/5 group">
                      <img src={image} alt="Target" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-90 group-hover:opacity-100" />
                      {!result && !isAnalyzing && (
                        <div className="absolute inset-0 bg-emerald-950/40 flex items-center justify-center backdrop-blur-[4px]">
                          <motion.button 
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={analyzeWaste}
                            className="bg-white text-emerald-950 px-10 py-6 rounded-full font-black text-xs uppercase tracking-[0.2em] flex items-center gap-3 shadow-2xl"
                      ...
                      {isAnalyzing && (
                        <div className="absolute inset-0 bg-[#FDFDFB]/95 flex flex-col items-center justify-center backdrop-blur-2xl">
                          <motion.div 
                            animate={{ rotate: 180, scale: [1, 1.05, 1] }}
                            transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
                            className="mb-10 w-24 h-24 rounded-full border-2 border-emerald-950/5 border-t-emerald-600 flex items-center justify-center"
                          >
                          ...
                          <div className="text-center space-y-3">
                            <p className="font-serif italic font-bold text-3xl text-emerald-950 tracking-tight">Merangkai Kreativitas...</p>
                  ...
                  <AnimatePresence>
                    {result && (
                      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                        ...
                        <div className="space-y-4">
                           {result.greeting && (
                             <div className="bg-emerald-50 border border-emerald-100 p-4 sm:p-6 rounded-[2rem] relative overflow-hidden">
                               <div className="absolute top-0 right-0 p-4 opacity-[0.03]">
                                 <Sparkles className="w-24 h-24 text-emerald-900" />
                               </div>
                               <p className="font-serif italic text-emerald-950 font-medium relative z-10 leading-relaxed text-sm sm:text-base">"{result.greeting}"</p>
                             </div>
                           )}
                           <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/70 backdrop-blur-xl p-5 sm:px-6 sm:py-3 rounded-[1.5rem] sm:rounded-full border border-emerald-950/5 shadow-sm shadow-emerald-900/5">
                              <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 min-w-0 flex-1">
                                <span className="text-emerald-950/50 text-[10px] font-black uppercase tracking-[0.3em] shrink-0">Terdeteksi</span>
                                <div className="hidden sm:block w-px h-4 bg-emerald-950/10"></div>
                                <span className="text-emerald-950 font-bold text-base sm:text-sm">{result.detected_item}</span>
                              </div>
                              <span className="inline-flex items-center justify-center self-start sm:self-auto px-5 py-2.5 rounded-full bg-emerald-950 text-white text-[10px] font-black uppercase tracking-widest shrink-0 shadow-md">{result.kategori}</span>
                           </div>
                        </div>
                        ...
                        <div className="space-y-6">
                          <div className="flex justify-between items-end px-2">
                             <div className="space-y-1">
                               <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-[0.4em]">Upcycle Strategies</p>
                               <h3 className="text-3xl font-bold font-serif text-emerald-950 italic">Pilihan Terbaik Untukmu</h3>
                             </div>
                             <div className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-black border border-emerald-100 uppercase tracking-widest">
                               SAW ranked
                             </div>
                          </div>
                            ...
                            {result.recommendations.map((opt, idx) => (
                              <motion.div 
                                ...
                                className={`p-8 rounded-[3rem] relative overflow-hidden transition-all duration-500 ${
                                  ...
                                  : 'bg-white border border-emerald-950/5 shadow-xl shadow-emerald-900/5'
                                }`}
                              >
                                <div className="flex justify-between items-start mb-6">
                                  <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className={`text-[10px] font-black uppercase tracking-widest ${idx === 0 ? 'text-emerald-400' : 'text-emerald-950/40'}`}>
                                ...
                                <p className={`text-lg leading-relaxed mb-10 italic ${idx === 0 ? 'text-white/60' : 'text-slate-500 group-hover:text-emerald-950'} transition-colors`}>
                                  {opt.description}
                                </p>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                  {[
                                    ...
                                    <div key={i} className={`p-4 rounded-[1.5rem] border ${idx === 0 ? 'bg-white/5 border-white/10' : 'bg-emerald-50 border-emerald-200/50'}`}>
                                       <p className={`text-[9px] font-black mb-2 uppercase tracking-widest text-center ${idx === 0 ? 'text-white/60' : 'text-emerald-700'}`}>{stat.label}</p>
                                       <div className="flex items-center justify-center gap-1">
                                          <span className={`text-sm font-bold ${idx === 0 ? 'text-white' : 'text-emerald-950'}`}>{stat.val}</span>
                                          <span className={`text-[9px] font-medium ${idx === 0 ? 'text-white/40' : 'text-emerald-600/70'}`}>/10</span>
                                       </div>
                                    </div>
                                  ))}
                                </div>
                        ...
                        <div className="space-y-6 bg-emerald-50/50 backdrop-blur-xl p-6 sm:p-8 rounded-[3rem] border border-emerald-100/50 shadow-sm relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-8 opacity-[0.05]">
                               <Leaf className="w-32 h-32 text-emerald-900" />
                            </div>
                            
                            <div className="space-y-1 relative z-10">
                               <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-[0.4em]">Environmental Impact</p>
                               <h3 className="text-3xl font-bold font-serif text-emerald-950 italic">Dampak Ekologismu</h3>
                            </div>

                            <p className="text-emerald-950 font-medium text-base sm:text-lg leading-relaxed relative z-10 italic border-l-4 border-emerald-400 pl-4 bg-white/60 p-4 rounded-r-2xl shadow-sm">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 flex-wrap relative z-10">
                               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-4 sm:p-5 rounded-[2rem] shadow-xl shadow-emerald-900/5 border border-emerald-950/5 flex flex-col items-center text-center gap-3">
                                 ...
                                 <div className="space-y-1">
                                    <p className="text-xl sm:text-2xl font-black text-emerald-950">{result.eco_impact?.water_saved || 0}</p>
                                    <p className="text-[8px] sm:text-[9px] font-bold text-slate-400/80 uppercase tracking-widest leading-tight">Liter<br/>Air</p>
                                 </div>
                               </motion.div>
                               
                               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white p-4 sm:p-5 rounded-[2rem] shadow-xl shadow-emerald-900/5 border border-emerald-950/5 flex flex-col items-center text-center gap-3">
                                 <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center border border-emerald-100"><Leaf className="w-6 h-6"/></div>
                                 <div className="space-y-1">
                                    <p className="text-xl sm:text-2xl font-black text-emerald-950">{result.eco_impact?.carbon_reduced || 0}</p>
                                    <p className="text-[8px] sm:text-[9px] font-bold text-slate-400/80 uppercase tracking-widest leading-tight">Kg<br/>CO2</p>
                                 </div>
                               </motion.div>
                               
                               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white p-4 sm:p-5 rounded-[2rem] shadow-xl shadow-emerald-900/5 border border-emerald-950/5 flex flex-col items-center text-center gap-3">
                                 ...
                                 <div className="space-y-1">
                                    <p className="text-xl sm:text-2xl font-black text-emerald-950">{result.eco_impact?.waste_diverted || 0}</p>
                                    <p className="text-[8px] sm:text-[9px] font-bold text-slate-400/80 uppercase tracking-widest leading-tight">Gram<br/>Sampah</p>
                                 </div>
                               </motion.div>

                               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white p-4 sm:p-5 rounded-[2rem] shadow-xl shadow-emerald-900/5 border border-emerald-950/5 flex flex-col items-center text-center gap-3">
                                 ...
                                 <div className="space-y-1">
                                    <p className="text-xl sm:text-2xl font-black text-emerald-950">{result.eco_impact?.energy_saved || 0}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <motion.button 
                            ...
                            whileTap={{ scale: 0.98 }} 
                            onClick={reset} 
                            className="bg-white border border-emerald-950/5 py-7 rounded-full font-black text-[10px] uppercase tracking-[0.3em] text-slate-400 hover:text-emerald-700 transition-all shadow-xl shadow-emerald-900/5 flex items-center justify-center gap-3"
                          ...
                          <motion.button 
                            ...
                            className={`py-7 rounded-full font-black text-[10px] uppercase tracking-[0.3em] flex items-center justify-center gap-3 shadow-2xl transition-all duration-500 ${
                              isCopied ? 'bg-emerald-950 text-emerald-400' : 'bg-emerald-600 text-white shadow-emerald-600/20'
                            }`}
            ...
            <motion.div key="gallery-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
               <div className="space-y-4">
                 <div className="inline-block px-4 py-1.5 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-black uppercase tracking-[0.3em]">
                   Inspirasi & Log
                 </div>
                 <h2 className="text-5xl font-bold font-serif text-emerald-950 italic">Karya Kamu</h2>
                 <p className="text-slate-500 text-lg font-medium">Bantu selamatkan bumi, satu karya upcycle di satu waktu.</p>
               </div>
               ...
               {isHistoryLoading ? (
                 <div className="grid gap-6">
                   {[1, 2, 3].map(i => (
                     <div key={i} className="bg-white rounded-[3rem] border border-emerald-950/5 overflow-hidden flex animate-pulse">
                       <div className="w-32 sm:w-48 h-32 sm:h-48 shrink-0 bg-emerald-50/50" />
                       <div className="p-6 sm:p-10 flex flex-col justify-center gap-4 flex-1">
                         <div className="w-20 h-4 bg-emerald-50 rounded-full" />
                         <div className="w-3/4 h-8 bg-emerald-50 rounded-lg" />
                         <div className="w-24 h-4 bg-emerald-50 rounded-full" />
                       </div>
                     </div>
                   ))}
                 </div>
               ) : history.length === 0 ? (
                 <div className="py-32 text-center bg-white rounded-[3.5rem] border border-dashed border-emerald-950/10 space-y-6 px-4">
                    ...
                    <div className="space-y-1">
                      <p className="text-emerald-950 font-bold font-serif text-2xl">Belum ada jejak terekam</p>
                      <p className="text-slate-400 font-medium tracking-tight">Mulailah dengan menscan sampah pertamamu.</p>
                    </div>
                 </div>
               ) : (
                 <div className="space-y-12">
                   <div className="bg-white p-6 sm:p-8 rounded-[3.5rem] border border-emerald-950/5 shadow-2xl shadow-emerald-950/10">
                     <div className="mb-6 space-y-1">
                       <h3 className="text-2xl font-bold font-serif text-emerald-950 italic">Dampak Ekologismu</h3>
                       <p className="text-sm font-medium text-slate-500">Pertumbuhan jumlah limbah yang berhasil dialihkan (Gram)</p>
                     </div>
                   ...
                   <div className="grid gap-6">
                     {history.map((item, idx) => (
                       <motion.div 
                        ...
                        key={item.id} 
                        className="bg-white/60 backdrop-blur-2xl rounded-[3rem] border border-white/80 overflow-hidden flex flex-col sm:flex-row shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-2xl hover:scale-[1.01] transition-all duration-700 group cursor-pointer p-2"
                     >
                        <div className="w-full sm:w-48 h-48 shrink-0 overflow-hidden relative rounded-[2.5rem]">
                        ...
                        <div className="p-6 sm:px-8 flex flex-col justify-center min-w-0 flex-1 relative overflow-hidden">
                           ...
                           <div className="flex flex-wrap items-center gap-2 mb-3 relative z-10">
                              <span className="text-[9px] font-black uppercase bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full border border-emerald-100">#{item.kategori}</span>
                           ...
                           <h4 className="text-xl sm:text-2xl font-bold text-emerald-950 font-serif line-clamp-1 italic relative z-10 mb-4">{item.recommendations?.[0]?.title}</h4>
                           <button className="px-6 py-3 bg-emerald-950 text-white rounded-full text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 self-start shadow-xl hover:shadow-emerald-950/20 hover:scale-105 transition-all" title="Buka detail proyek ini">
            ...
            <motion.div key="banks-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12 relative overflow-x-clip px-0.5">
               ...
               <div className="space-y-4 relative z-10">
                 ...
                 <h2 className="text-5xl font-bold font-serif text-emerald-950 italic leading-tight">Pusat Pengolahan<br/>Terdekat</h2>
                 <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-sm">Temukan bank sampah & pusat daur ulang di sekitarmu dalam satu ketukan.</p>
               </div>

               <div className="bg-white p-10 rounded-[3.5rem] border border-emerald-950/5 shadow-2xl shadow-emerald-950/10 flex flex-col items-center text-center space-y-10 relative overflow-hidden z-10">
                 <div className="w-24 h-24 rounded-full bg-emerald-50 flex items-center justify-center relative">
                   ...
                   />
                   <div className="w-16 h-16 rounded-full bg-emerald-950 flex items-center justify-center relative z-10 shadow-xl">
                 ...
                 {locationError ? (
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full space-y-6">
                     ...
                     <motion.button
                       ...
                       className="w-full py-6 rounded-full bg-emerald-950 text-white font-black text-[10px] uppercase tracking-[0.3em] flex items-center justify-center gap-3 shadow-2xl"
                   ...
                   >
                     {nearbyBanks.map((bank, index) => (
                       <motion.div 
                        ...
                        key={bank.id} 
                        className="bg-white p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] border border-emerald-950/5 shadow-xl shadow-emerald-900/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 group hover:scale-[1.01] transition-all duration-500 overflow-hidden"
                       >
                         <div className="flex items-start sm:items-center gap-5 sm:gap-6 w-full sm:min-w-0 sm:flex-1">
                            <div className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 rounded-[1.25rem] sm:rounded-[1.5rem] bg-emerald-50 flex items-center justify-center text-emerald-950 border border-emerald-100 group-hover:bg-emerald-950 group-hover:text-white transition-all duration-500">
                               <MapPin className="w-7 h-7 sm:w-8 sm:h-8" />
                            </div>
                            <div className="min-w-0 flex-1 space-y-1 sm:space-y-2">
                               <h4 className="text-lg sm:text-xl font-bold text-emerald-950 font-serif italic truncate leading-tight">{bank.name}</h4>
                               <div className="flex items-center gap-3">
                                 <div className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[9px] sm:text-[10px] font-black border border-emerald-100 uppercase tracking-widest shrink-0">{bank.distance}</div>
                         ...
                         <motion.button 
                           ...
                           className="w-full sm:w-auto px-8 py-4 bg-emerald-950 text-white rounded-full font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-xl hover:shadow-emerald-950/20 transition-all shrink-0"
      ...
      <div className="fixed bottom-8 left-0 right-0 z-50 px-8">
        <nav className="max-w-xs mx-auto bg-white/70 backdrop-blur-3xl p-2 rounded-full border border-emerald-950/5 shadow-2xl flex items-center justify-between">
          {[
            ...
            >
              <tab.icon className={`w-5 h-5 transition-all duration-500 ${activeTab === tab.id ? 'text-white scale-110' : 'text-emerald-950/40 group-hover:text-emerald-950/70'}`} />
              <div className={`absolute -bottom-8 transition-all duration-500 ${activeTab === tab.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-950/40">{tab.label}</span>
              </div>
              {activeTab === tab.id && (
                <motion.div 
